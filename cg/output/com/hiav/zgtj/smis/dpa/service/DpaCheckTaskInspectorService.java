...
@Service
public class DpaCheckTaskInspectorService extends BaseService<DpaCheckTaskInspector> {
	...
	@Override
	public void find(Page<DpaCheckTaskInspector> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_check_task_inspector e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaCheckTaskInspector.class);
	...
	public Collection<DpaCheckTaskInspector> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaCheckTaskInspector x = (DpaCheckTaskInspector) this.getCurrentSession().get(
				DpaCheckTaskInspector.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaCheckTaskInspector> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaCheckTaskInspector x : datas) {
