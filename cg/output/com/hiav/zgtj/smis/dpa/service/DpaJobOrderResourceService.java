...
@Service
public class DpaJobOrderResourceService extends BaseService<DpaJobOrderResource> {
	...
	@Override
	public void find(Page<DpaJobOrderResource> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_job_order_resource e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaJobOrderResource.class);
	...
	public Collection<DpaJobOrderResource> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaJobOrderResource x = (DpaJobOrderResource) this.getCurrentSession().get(
				DpaJobOrderResource.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaJobOrderResource> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaJobOrderResource x : datas) {
