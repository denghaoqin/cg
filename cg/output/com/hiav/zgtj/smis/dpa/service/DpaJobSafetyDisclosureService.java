...
@Service
public class DpaJobSafetyDisclosureService extends BaseService<DpaJobSafetyDisclosure> {
	...
	@Override
	public void find(Page<DpaJobSafetyDisclosure> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_job_safety_disclosure e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaJobSafetyDisclosure.class);
	...
	public Collection<DpaJobSafetyDisclosure> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaJobSafetyDisclosure x = (DpaJobSafetyDisclosure) this.getCurrentSession().get(
				DpaJobSafetyDisclosure.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaJobSafetyDisclosure> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaJobSafetyDisclosure x : datas) {
