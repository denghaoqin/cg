...
@Service
public class DpaJobConstructionService extends BaseService<DpaJobConstruction> {
	...
	@Override
	public void find(Page<DpaJobConstruction> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_job_construction e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaJobConstruction.class);
	...
	public Collection<DpaJobConstruction> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaJobConstruction x = (DpaJobConstruction) this.getCurrentSession().get(
				DpaJobConstruction.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaJobConstruction> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaJobConstruction x : datas) {
