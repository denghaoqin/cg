...
@Service
public class DpaHiddenLibraryService extends BaseService<DpaHiddenLibrary> {
	...
	@Override
	public void find(Page<DpaHiddenLibrary> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_hidden_library e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaHiddenLibrary.class);
	...
	public Collection<DpaHiddenLibrary> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaHiddenLibrary x = (DpaHiddenLibrary) this.getCurrentSession().get(
				DpaHiddenLibrary.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaHiddenLibrary> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaHiddenLibrary x : datas) {
