...
@Service
public class DpaSafetyCheckItemService extends BaseService<DpaSafetyCheckItem> {
	...
	@Override
	public void find(Page<DpaSafetyCheckItem> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_safety_check_item e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaSafetyCheckItem.class);
	...
	public Collection<DpaSafetyCheckItem> find(Map<String, Object> params,
			Criteria filterCriteria) {
	...
	public void delete(String id) {
		DpaSafetyCheckItem x = (DpaSafetyCheckItem) this.getCurrentSession().get(
				DpaSafetyCheckItem.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaSafetyCheckItem> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaSafetyCheckItem x : datas) {
