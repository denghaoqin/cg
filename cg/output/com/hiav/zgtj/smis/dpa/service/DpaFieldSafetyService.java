...
@Service
public class DpaFieldSafetyService extends BaseService<DpaFieldSafety> {
	...
	@Override
	public void find(Page<DpaFieldSafety> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_field_safety e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaFieldSafety.class);
	...
	public Collection<DpaFieldSafety> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaFieldSafety x = (DpaFieldSafety) this.getCurrentSession().get(
				DpaFieldSafety.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaFieldSafety> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaFieldSafety x : datas) {
