...
@Service
public class DpaHiddenDangerRectificationService extends BaseService<DpaHiddenDangerRectification> {
	...
	@Override
	public void find(Page<DpaHiddenDangerRectification> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_hidden_danger_rectification e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaHiddenDangerRectification.class);
	...
	public Collection<DpaHiddenDangerRectification> find(Map<String, Object> params,
			Criteria filterCriteria) {
	...
	public void delete(String id) {
		DpaHiddenDangerRectification x = (DpaHiddenDangerRectification) this.getCurrentSession().get(
				DpaHiddenDangerRectification.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaHiddenDangerRectification> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaHiddenDangerRectification x : datas) {
