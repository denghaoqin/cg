...
@Service
public class DpaCheckTaskItemService extends BaseService<DpaCheckTaskItem> {
	...
	@Override
	public void find(Page<DpaCheckTaskItem> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_check_task_item e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaCheckTaskItem.class);
	...
	public Collection<DpaCheckTaskItem> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaCheckTaskItem x = (DpaCheckTaskItem) this.getCurrentSession().get(
				DpaCheckTaskItem.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaCheckTaskItem> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaCheckTaskItem x : datas) {
