...
@Service
public class DpaHiddenLibraryMeasureService extends BaseService<DpaHiddenLibraryMeasure> {
	...
	@Override
	public void find(Page<DpaHiddenLibraryMeasure> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_hidden_library_measure e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaHiddenLibraryMeasure.class);
	...
	public Collection<DpaHiddenLibraryMeasure> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaHiddenLibraryMeasure x = (DpaHiddenLibraryMeasure) this.getCurrentSession().get(
				DpaHiddenLibraryMeasure.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaHiddenLibraryMeasure> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaHiddenLibraryMeasure x : datas) {
