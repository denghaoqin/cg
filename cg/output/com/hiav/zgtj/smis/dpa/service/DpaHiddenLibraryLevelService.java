...
@Service
public class DpaHiddenLibraryLevelService extends BaseService<DpaHiddenLibraryLevel> {
	...
	@Override
	public void find(Page<DpaHiddenLibraryLevel> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_hidden_library_level e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaHiddenLibraryLevel.class);
	...
	public Collection<DpaHiddenLibraryLevel> find(Map<String, Object> params,
			Criteria filterCriteria) {
	...
	public void delete(String id) {
		DpaHiddenLibraryLevel x = (DpaHiddenLibraryLevel) this.getCurrentSession().get(
				DpaHiddenLibraryLevel.class, id);
		x.setDeleted(true);
				this.update(x);
	}
	...
	public void save(Collection<DpaHiddenLibraryLevel> datas) {
		...
		for (DpaHiddenLibraryLevel x : datas) {
