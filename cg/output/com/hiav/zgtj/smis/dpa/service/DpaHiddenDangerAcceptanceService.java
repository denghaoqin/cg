...
@Service
public class DpaHiddenDangerAcceptanceService extends BaseService<DpaHiddenDangerAcceptance> {
	...
	@Override
	public void find(Page<DpaHiddenDangerAcceptance> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_hidden_danger_acceptance e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaHiddenDangerAcceptance.class);
	...
	public Collection<DpaHiddenDangerAcceptance> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaHiddenDangerAcceptance x = (DpaHiddenDangerAcceptance) this.getCurrentSession().get(
				DpaHiddenDangerAcceptance.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaHiddenDangerAcceptance> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaHiddenDangerAcceptance x : datas) {
