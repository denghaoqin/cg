...
@Service
public class DpaSafetyCheckItemLineService extends BaseService<DpaSafetyCheckItemLine> {
	...
	@Override
	public void find(Page<DpaSafetyCheckItemLine> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_safety_check_item_line e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaSafetyCheckItemLine.class);
	...
	public Collection<DpaSafetyCheckItemLine> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaSafetyCheckItemLine x = (DpaSafetyCheckItemLine) this.getCurrentSession().get(
				DpaSafetyCheckItemLine.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaSafetyCheckItemLine> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaSafetyCheckItemLine x : datas) {
