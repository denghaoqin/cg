...
@Service
public class DpaJobLateCallService extends BaseService<DpaJobLateCall> {
	...
	@Override
	public void find(Page<DpaJobLateCall> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_job_late_call e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaJobLateCall.class);
	...
	public Collection<DpaJobLateCall> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaJobLateCall x = (DpaJobLateCall) this.getCurrentSession().get(
				DpaJobLateCall.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaJobLateCall> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaJobLateCall x : datas) {
