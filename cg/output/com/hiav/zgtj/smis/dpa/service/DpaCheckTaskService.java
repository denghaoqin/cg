...
@Service
public class DpaCheckTaskService extends BaseService<DpaCheckTask> {
	...
	@Override
	public void find(Page<DpaCheckTask> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_check_task e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaCheckTask.class);
	...
	public Collection<DpaCheckTask> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaCheckTask x = (DpaCheckTask) this.getCurrentSession().get(
				DpaCheckTask.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaCheckTask> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaCheckTask x : datas) {
