...
@Service
public class DpaSafetyDisclosureService extends BaseService<DpaSafetyDisclosure> {
	...
	@Override
	public void find(Page<DpaSafetyDisclosure> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_safety_disclosure e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaSafetyDisclosure.class);
	...
	public Collection<DpaSafetyDisclosure> find(Map<String, Object> params,
			Criteria filterCriteria) {
	...
	public void delete(String id) {
		DpaSafetyDisclosure x = (DpaSafetyDisclosure) this.getCurrentSession().get(
				DpaSafetyDisclosure.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaSafetyDisclosure> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaSafetyDisclosure x : datas) {
