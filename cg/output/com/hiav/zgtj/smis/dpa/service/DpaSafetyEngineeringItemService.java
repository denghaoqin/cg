...
@Service
public class DpaSafetyEngineeringItemService extends BaseService<DpaSafetyEngineeringItem> {
	...
	@Override
	public void find(Page<DpaSafetyEngineeringItem> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_safety_engineering_item e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaSafetyEngineeringItem.class);
	...
	public Collection<DpaSafetyEngineeringItem> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaSafetyEngineeringItem x = (DpaSafetyEngineeringItem) this.getCurrentSession().get(
				DpaSafetyEngineeringItem.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaSafetyEngineeringItem> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaSafetyEngineeringItem x : datas) {
