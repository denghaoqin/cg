...
@Service
public class DpaSafetyMgmtWorkEndService extends BaseService<DpaSafetyMgmtWorkEnd> {
	...
	@Override
	public void find(Page<DpaSafetyMgmtWorkEnd> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_safety_mgmt_work_end e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaSafetyMgmtWorkEnd.class);
	...
	public Collection<DpaSafetyMgmtWorkEnd> find(Map<String, Object> params,
			Criteria filterCriteria) {
	...
	public void delete(String id) {
		DpaSafetyMgmtWorkEnd x = (DpaSafetyMgmtWorkEnd) this.getCurrentSession().get(
				DpaSafetyMgmtWorkEnd.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaSafetyMgmtWorkEnd> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaSafetyMgmtWorkEnd x : datas) {
