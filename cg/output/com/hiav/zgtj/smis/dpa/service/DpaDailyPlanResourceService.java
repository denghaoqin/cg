...
@Service
public class DpaDailyPlanResourceService extends BaseService<DpaDailyPlanResource> {
	...
	@Override
	public void find(Page<DpaDailyPlanResource> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_daily_plan_resource e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaDailyPlanResource.class);
	...
	public Collection<DpaDailyPlanResource> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaDailyPlanResource x = (DpaDailyPlanResource) this.getCurrentSession().get(
				DpaDailyPlanResource.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaDailyPlanResource> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaDailyPlanResource x : datas) {
