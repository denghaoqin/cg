...
@Service
public class DpaJobToolsService extends BaseService<DpaJobTools> {
	...
	@Override
	public void find(Page<DpaJobTools> page, Map<String, Object> params,
			Criteria filterCriteria) {

		StringBuilder sql = new StringBuilder(" select e.* FROM dpa_job_tools e ")
			//.append(" left join pm_project p on e.project_id = p.id ")
			.append(" where e.deleted=0 ");
		Map<String, Class> e = new HashMap<String, Class>();
		e.put("e", DpaJobTools.class);
	...
	public Collection<DpaJobTools> find(Map<String, Object> params) {
	...
	public void delete(String id) {
		DpaJobTools x = (DpaJobTools) this.getCurrentSession().get(
				DpaJobTools.class, id);
		x.setDeleted(true);
				this.update(x);
	}

	@Override
	public void save(Collection<DpaJobTools> datas) {
		IUser marker = ContextHolder.getLoginUser();
		Date markTime = new Date();
		Session session = this.getCurrentSession();
		
		for (DpaJobTools x : datas) {
